...
def test_sets_alt_hold_on_start_and_starts_in_coarse(ros_context):
    from orchestrator.docking_fsm_node import DockingFSM
    io = _FakeIO()
    node = DockingFSM(vehicle_io=io, parameter_overrides=_load_params())
    harness = _Harness()
    _spin(node, harness, lambda i: harness.coarse(ready=False), iterations=10)
    assert "ALT_HOLD" in io.modes
    assert harness.states and harness.states[0] == DockingState.COARSE
...
def test_coarse_to_fine_on_handoff(ros_context):
    ...
    node = DockingFSM(vehicle_io=io, parameter_overrides=_load_params())
    harness = _Harness()
    _spin(node, harness, lambda i: harness.coarse(ready=True), iterations=15)
...
def test_fine_to_docked_disarms(ros_context):
    ...
    assert harness.states[-1] == DockingState.DOCKED
    assert io.arms and io.arms[-1] is False, "DOCKED must disarm"
    node.destroy_node(); harness.destroy_node()
