...
def test_ki_zero_default_no_integral_growth():
    # Default ki=0: repeated constant error must NOT grow the command (pure PD).
    c = controller(kp_sway=0.5, kd_sway=0.0)
    a = c.step(np.array([0.5, 0.1, 0.0]), 0.0, dt=0.1)
    b = c.step(np.array([0.5, 0.1, 0.0]), 0.0, dt=0.1)
    assert a.sway == pytest.approx(0.05)
    assert b.sway == pytest.approx(0.05)


def test_integral_accumulates_on_constant_cross_track_error():
    c = controller(kp_sway=0.5, kd_sway=0.0, ki_sway=1.0, i_max=10.0, v_max_sway=10.0)
    err = np.array([0.5, 0.1, 0.0])  # range at handoff (surge=0), constant lateral 0.1
    first = c.step(err, 0.0, dt=0.1)
    # int = 0.1*0.1 = 0.01 -> sway = 0.5*0.1 + 1.0*0.01 = 0.06
    assert first.sway == pytest.approx(0.06)
    second = c.step(err, 0.0, dt=0.1)
    # int = 0.02 -> sway = 0.05 + 0.02 = 0.07
    assert second.sway == pytest.approx(0.07)


def test_integral_accumulator_clamped_by_i_max():
    c = controller(kp_sway=0.0, kd_sway=0.0, ki_sway=1.0, i_max=0.05, v_max_sway=10.0)
    for _ in range(20):  # error 1.0, dt 0.1 -> would grow 0.1/step without the clamp
        cmd = c.step(np.array([0.5, 1.0, 0.0]), 0.0, dt=0.1)
    # accumulator clamped at i_max=0.05 -> sway = ki * 0.05 = 0.05
    assert cmd.sway == pytest.approx(0.05)


def test_integral_on_heave():
    c = controller(kp_heave=0.0, kd_heave=0.0, ki_heave=1.0, i_max=10.0, v_max_heave=10.0)
    c.step(np.array([0.5, 0.0, 0.1]), 0.0, dt=0.1)  # int_vertical = 0.01
    cmd = c.step(np.array([0.5, 0.0, 0.1]), 0.0, dt=0.1)  # int_vertical = 0.02
    assert cmd.heave == pytest.approx(1.0 * 0.02)


def test_reset_clears_integral():
    c = controller(kp_sway=0.0, kd_sway=0.0, ki_sway=1.0, i_max=10.0, v_max_sway=10.0)
    for _ in range(5):
        c.step(np.array([0.5, 0.2, 0.0]), 0.0, dt=0.1)
    c.reset()
    cmd = c.step(np.array([0.5, 0.2, 0.0]), 0.0, dt=0.1)
    # after reset, int = 0.2*0.1 = 0.02 -> sway = 1.0 * 0.02
    assert cmd.sway == pytest.approx(0.02)
